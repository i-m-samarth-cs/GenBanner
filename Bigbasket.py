...
class PromotionalContentGenerator:
    ...
    def draw_diya(self, draw):
        # Simplified diya drawing for Diwali
    ...
    def draw_flag(self, draw):
        # Simplified Indian flag drawing
    ...
    def draw_sun(self, draw):
        # Simplified sun for summer sale
    ...
    def generate_banner(self):
        ...
        if self.product_images:
            num_products = random.randint(2, 4)  # Randomly choose how many product images to display
            x_pos = 50
            for i in range(num_products):
                ...
                x_pos += product_image.width + 20  # Adjust the x-position for the next product image

        # Apply theme
        theme = random.choice(self.themes)
        self.apply_theme(background, theme)

        return background

    def convert_banners_to_video(self, video_filename, num_banners=10, fps=1):
        # Generate banners and convert them directly to video
        clips = []
        for _ in range(num_banners):
            banner = self.generate_banner()
            clip = mp.ImageClip(np.array(banner))
            clip = clip.set_duration(1 / fps)  # Set each banner to appear for 1 second
            clips.append(clip)
...
def main():
    ...
    video_filename = f'C:/Users/samar/Desktop/Google Gen AI/promotional_video_{random_number}.mp4'
